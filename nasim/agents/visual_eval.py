...
def main():
    ...
    env = nasim.load(f"../scenarios/own_S/Permutations/{args.envname}", flat_actions=True, flat_obs=True)
    #eval_stats = experiments.eval_performance(env=env,agent=agent,n_steps=None,n_episodes=args.eval_n_runs,max_episode_len=timestep_limit)
    logger = None
    ...
    while not terminate:
        ...
        episode_len += 1
        timestep += 1
        reset = (done or episode_len == timestep_limit
        ...
        terminate = len(scores) >= args.eval_n_runs
    ...
    print('n_runs: {} mean: {} median: {} stdev {}'.format(
        args.eval_n_runs, stats['mean'], stats['median'],
        stats['stdev']))
